...
object WindowCounter {
  def main(args: Array[String]) {
    ...
    //val wordCounts2 = words.map(x => (x , 1)).reduceByKeyAndWindow("", windowDuration)
   ...
   // sortedWordCount.foreachRDD(foreachFunc)
   val data= sortedWordCount.transform{
      
      line=>
     
      val data =line.reduceByKey(_+_)
