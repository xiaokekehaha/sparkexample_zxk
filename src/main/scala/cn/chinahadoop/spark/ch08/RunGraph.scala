...
object RunGraph extends Serializable {
  ...
  def loadMedline(sc: SparkContext, path: String): RDD[String] = {
    ...
    conf.set("", "<MedlineCitation ")
    conf.set("", "</MedlineCitation>")
    /* val in = sc.newAPIHadoopFile(path, classOf[XmlInputFormat],
      classOf[LongWritable], classOf[Text], conf)*/

    val file2 = sc.newAPIHadoopFile[LongWritable, Text, TextInputFormat]("hdfs://192.168.100.130:8020/user/hue/pig/examples/data/sonnets.txt")
    file2.map(line => line._2.toString)
  }
