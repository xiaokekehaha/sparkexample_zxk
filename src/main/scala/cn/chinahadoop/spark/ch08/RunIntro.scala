...
object RunIntro extends Serializable {
  ...
  /*     iter.map{
     ...
     // val data3=Seq(data2).iterator
     // val data = Iterator(data2)
      Iterator(nas)
    }}

    val nastats2 = rdd.mapPartitions(
      iter => {

        val data2 = iter.map {

          val filled = new ArrayBuffer[Double]()

          line =>
            val data = line(0)
            for (data1 <- 0 until line.length) {

              filled(data1) = line(data1)
            }
    ...
    /* \\   val nastats2=rdd.mapPartitions{
     ...
     line=>{
       ...
       val data=line.map{
          ...
          line2=>
           val data=line2.map(d=>NAStatCounter(d)) 
            data
        }
       line.foreach{
         arr=>{
              data.zip(arr).foreach{
